...
def val(net, dataset, criterion, max_iter=100):
    ...
    for i in range(max_iter):
        ...
        #cost = criterion(preds.cpu(), text.cpu(), preds_size.cpu(), length.cpu()).cuda()
        cost = criterion(preds, text, preds_size, length).cuda()
        loss_avg.add(cost)
...
def trainBatch(net, criterion, optimizer, idx):
...
for epoch in range(opt.nepoch):
    ...
    while i < len(train_loader):
        ...
        cost = trainBatch(crnn, criterion, optimizer, i % 2)
        loss_avg.add(cost)
        i += 1
